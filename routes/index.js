...
router.get('/timeLife', estaLogeado, function(req, res, next) {
  ...
  console.log(user);
    sql = client.query("SELECT u_permiso, u_username as name, u_surname as ape, u_photo as image FROM app_kanban.users WHERE u_email = ($1)", [user], function(err, result) {
      if (err) {
          console.log(err);
          return;
       }else{
          ...
          if (data[0].u_permiso === 0){
             res.render('admin/timeLife.html');
          }else if (data[0].u_permiso === 1) {
             res.render('admin/timeLife_emple.html');
          }else{
            res.render('admin/timeLife_gerente.html');
          }
       }
   });
      ...
      sql = client.query("SELECT u_permiso, u_username as name, u_surname as ape, u_photo as image FROM app_kanban.users WHERE u_email = ($1)", [user], function(err, result) {
        if (err) {
            ...
            if (data[0].u_permiso === 0){
               ...
               res.render('admin/calendario_emple.html');
            }else{
              res.render('admin/index_gerente.html');
            }
         }
     });
});
      ...
      sql = client.query("SELECT u_permiso, u_username as name, u_surname as ape, u_photo as image FROM app_kanban.users WHERE u_email = ($1)", [user], function(err, result) {
        if (err) {
            ...
            if (data[0].u_permiso === 0){
               ...
               res.render('admin/perfil_emple.html');
            }else{
              res.render('admin/perfil_gerente.html');
            }
         }
     });
    //res.render('admin/perfil.html');
      ...
      sql = client.query("SELECT u_permiso, u_username as name, u_surname as ape, u_photo as image FROM app_kanban.users WHERE u_email = ($1)", [user], function(err, result) {
        if (err) {
            ...
            if (data[0].u_permiso === 0){
              ...
              res.render('admin/index_emple.html');
            }
         }
     });
});
