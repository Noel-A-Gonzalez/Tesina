var app = function(){
	...
	*/
	...
	/*
		Crea las tareas fantasmas en los lugares que faltan
	*/
	var positGhost = function(){
		$(".lobilist-items").each(function(index) {
	        var listgetItem = "#" + $($(this)[0]).parents().eq(1).attr("id");
                cantPosit = $(listgetItem + " ul .nowait").size();
                cantMax = $(listgetItem + " #maxItem").text();
                valueActItem = parseInt(cantMax, 8);
            if (cantPosit < valueActItem) {
                $(listgetItem).find("ul").append("<div class='positGhost'></div>")
            }else{
            	$(listgetItem).find("ul>.positGhost").remove();
            }
		});
	}
