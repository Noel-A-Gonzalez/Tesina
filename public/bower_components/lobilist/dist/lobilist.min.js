...
//------------------------------------------------------------------------------
        ...
        this.saveOrUpdateItem = function (item, errorCallback) {
            if (item.id) {
                ...
                console.log();
                listID = '#' + me.$el[0].id;
                cantPosit = $(listID + " ul .nowait").size();
                cantMax = $(listID + " #maxItem").text();
                valueActItem = parseInt(cantMax, 8);

                console.log(valueActItem);
                if (cantPosit < valueActItem || isNaN(valueActItem) ) {
                    
                        _addItemSave(item, errorCallback)
                        //app.positGhost();
                }else{
                    Lobibox.alert("info",{
                        title: "Información",
                        msg: "La columna ya se encuentra completa en estos momentos.."
                    });
                }
               // ;
        ...
        this.finishTitleEditing = function () {
            ...
            /*llama astatus.editheader*/
            column.editHeader(me.$el[0].id, $(me.$el[0]).find(".lobilist-title")[0].innerHTML);
            $input.remove();
        ...
        this.editItem = function (id) {
            ...
            me.$el.find('.hasDatepicker').prop("disabled", false);
            me.$el.find('form [name=dueDate]').prop("disabled", false);
...
//------------------------------------------------------------------------------
        ...
        function _createTitle() {
            ...
            /*
                Crea el div con los span que muestra la cantidad de item
                de cada lista
            */
            var span1 = $('<span />').attr("id", "countIt").addClass('label label-default').html("0 de");
            var span2 = $('<span />').attr("id", "maxItem").addClass('label label-default').html(" 3");
            var $cantItem = $('<div>', {
                'class': 'cantItem',
                'style': 'text-align:center',
                'html' : [span1, span2],
                dblclick: function(){
                    var idList = $(this).parents().eq(1).attr('id')
                    posit.editWIP(idList);
                  }
            }).appendTo($header);
        ...
        function _submitForm() {
            if (!$form[0].title.value) {
                _showFormError('title', 'El título no puede estar vacío.');
                return;
            }
            me.saveOrUpdateItem({
                id: $form[0].id.value,
        ...
        function _createFooter() {
            ...
            $('<button>', {
                ...
                me.$el.find('.hasDatepicker').prop("disabled", true);
                me.$el.find('form [name=dueDate]').prop("disabled", true);
                _resetForm();
                $form.removeClass('hide');
                $footer.addClass('hide');
            }).appendTo($footer);
            $('<i>', {
                'class': 'fa fa-spinner fa-pulse fa-lg loading'
            }).appendTo($footer)
            me.$el.append($footer);
            return $footer;
        }
         ...
         * a|nd initial items are added to the list
         *
         * @type Object
         */
        function _addItem(item) {
            //var cantID = $(".lobilist-wrapper ul li").size();
            if (!item.id) {
                item.id = me.$lobiList.getNextId();
            }
            if (_triggerEvent('beforeItemAdd1', [me, item]) !== false) {
                item = _processItemData(item);
                _addItemToList(item);
            }
        }
        ...
        /**
         * Se activa cuando presiona el boton guardar
         * 
         *
         * @type Object
         */
        function _addItemSave(items) {
            $.get("ritem/getNextID", function(){
            })
            .done(function(data) {
                var cantID = $(".lobilist-wrapper ul li").size();
                if (!items.id) {
                    items.id = data[0].nextid;//me.$lobiList.getNextId();
                }
                if (_triggerEvent('beforeItemAdd', [me, items]) !== false) {
                    items = _processItemData(items);
                    var fecha = items.dueDate + ' 00:00:01 am'; 
                    var itemJ = {"item":
                                    {
                                    "id": items.id,
                                    "title": items.title,
                    ...
                    if (items.dueDate) {
                        itemJ.item.date = fecha;
                    }  
                    
                    column.showProcess(me.$el[0].id);
                   $.post("ritem/saveItem", {id: itemJ.item.id, tit:itemJ.item.title, desc:itemJ.item.description, date:itemJ.item.date, list: itemJ.item.list}, function() {
                    })
                    .done(function() {
                        _addItemToList(items);
                        //app.positGhost();
                        column.hideProcess(me.$el[0].id);
                    })
                    .fail(function() {
                       Lobibox.alert("error",{
                            title: "ERROR",
                ...
                $('<div class="' + st + '"></div>')
                    ...
                    .click(function () {
                        me.$el.removeClass(me.$globalOptions.listStyles.join(" "))
                           ...
                           column.editColor(me.$el[0].id, this.className);
                    })
                    .appendTo($menu);
            }
            return $dropdown;
        ...
        function _onRemoveListClick() {
            ...
            var id = me.$el[0].id;
                cantPosit = $("#" + id).find(" ul li").size();
                indexList = $("#"+id).parent().index();
                sizeList = $(".lobilist-wrapper").size()-1;

                if (indexList === 0 || indexList === sizeList) {
                    Lobibox.alert("info",{
                        title: "Información",
                        msg: "Lo siento, esta columna no esta permitido para eliminar..",
                        shown: function(lobibox){
                            var snd = new Audio('bower_components/lobibox/sounds/sound5.ogg');
                            snd.play();
                        },
                    });
                }else{
                    if (cantPosit > 0) {
                        Lobibox.alert("info",{
                            title: "Información",
                            msg: "No se pudo eliminar. Hay tareas por hacer en este estado..",
                        }); 
                    }else{
                        $.post("rstatus/deleteList",{idlist: id})
                        .done(function() {
        ...
        function _enableSorting() {
            me.$el.find('.lobilist-items').sortable({
                ...
                update: function (event, ui) {
                    ...
                    //console.log($(".lobilist-body>ul>li").get(0));
                   // console.log(ids);
                    _triggerEvent('afterItemReorder', [me, ui.item]);
                },
                receive: function(event, ui) {
                    var indexOrigen = parseInt($(ui.sender[0]).parents().eq(2).index(), 8)+1;
                        indexDest = parseInt($($(this)[0]).parents().eq(2).index(), 8)+1;
                        nextListID = '#' + $(".lobilist").eq(indexDest-1).attr("id");
                        cantPosit = $(nextListID + " ul .nowait").size();
                        cantMax = $(nextListID + " #maxItem").text();
                        valueActItem = parseInt(cantMax, 8);
                        positID = $(ui.item[0]).data("id");
                        datehoy = new Date();
                        fecha = moment(datehoy).format('DD-MM-YYYY');

                    if (cantPosit <= valueActItem || isNaN(valueActItem)) {
                        var nextList = $(".lobilist").eq(indexDest-1).attr("id");
                            listOrigen = $(".lobilist").eq(indexOrigen-1).attr("id");
                        column.showProcess(nextList);
                        $.post("ritem/updateStatus", {positID: positID, listOrigenID: listOrigen,  listID: nextList, fecha: fecha})
                        .done(function(){
                            column.hideProcess(nextList);
                            posit.updateState(indexOrigen, indexDest);
                        })
                        .fail(function(){
                        ...
                        Lobibox.alert("info",{
                            title: "Información",
                            msg: "La columna ya está completa.."
                        }); 
                    }              
        ...
        function _addItemToList(item) {
            ...
            if (item.dueDate) {
                $li.append($('<div>', {
                    ...
                    html: posit.convertDate(item.dueDate)
                }));
            }
        ...
        function _addItemControls($li) {
            ...
            if (me.$options.enableTodoRemove) {
                $itemControlsDiv.append($('<div>', {
                    ...
                    'title': 'Eliminar Posit',
                    html: '<i class="glyphicon glyphicon-remove"></i>'
                }).click(function () {
                    var idItem = $(this).closest('li').data('lobiListItem');
                    Lobibox.confirm({
                        title: "Confirmar",
                        buttons: {
                            yes: {
                        ...
                        msg: "¿Seguro que quiere eliminar este Posit?",
                        callback: function ($this, type, ev) {
                            if (type == "yes") {
                                _onDeleteItemClick(idItem);
        ...
        function _updateItemInList(item) {
            var dueFecha = moment(item.dueDate, 'MM.DD.YYYY').format('DD/MM/YYYY');
            var fecha = dueFecha + ' 00:00:01 am';
            if (item.dueDate) {
                item.date = fecha;
            }else{
                delete item.dueDate;
                $('li[data-id="'+item.id+'"]').addClass("nowait").removeClass("positWait");
                $("#"+item.id).find(".lobilist-item-duedate").remove();
            }
                column.showProcess(me.$el[0].id);
                $.post("ritem/updateItem", {itemid: item.id, itemtit: item.title, itemdescr: item.description, itemdate: item.date})
                .done(function(res) {
                    ...
                    if (item.dueDate) {
                        $li.append('<div class="lobilist-item-duedate">' + posit.convertDate(item.dueDate) + '</div>');
                    }
                    $li.data('lobiListItem', item);
                    _triggerEvent('afterItemUpdate', [me, item]);

                    var eventDate = posit.convertDateCalendar(item.dueDate);
                    if (item.dueDate) {
                        $.get("/auth/calendar",{positId: item.id, titleEvent: item.title, descriptEvent: item.description, fecha: eventDate}, function(data){
                            if (data){
        ...
        function _removeItemFromList(item) {
            column.showProcess(me.$el[0].id);
            $.post("ritem/deleteItem", {item: item.id})
            .done(function() {
...
//------------------------------------------------------------------------------
        ...
        this.addList1 = function (list) {
            if (!(list instanceof List)) {
                list = new List(me, _processListOptions(list));
            }
            if (_triggerEvent('beforeListAdd1', [me, list]) !== false) {
               
               var sizeList = $(".lobilist-wrapper").size()-1;
               var lastListID = $(".lobilist-wrapper").eq(sizeList).children().attr("id");
               
               list.$elWrapper.insertBefore($("#" + lastListID).parent());
               $('input,form').attr('autocomplete','off');
               app.form_sin_enter();
                list.$el.data('lobiList', list);

               _triggerEvent('afterListAdd1', [me, list]);
